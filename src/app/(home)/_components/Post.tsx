type Props = {
  ...
  title: string;
  content: string;
  author: string; // replace with author object
}

const calculateDays = (date: Date): string => {
  const todaysDate = new Date();
  const milisecDfifference = todaysDate.getTime() - date.getTime();
  const milisecPerDay = 1000 * 60 * 60 * 24;
  const numberOfDays = Math.floor(milisecDfifference / milisecPerDay);
...
export default function Post({ id, type="Question", date, title, content, author }: Props) {
  const daysAgo = calculateDays(date);
